...
class TestWeatherApp(TestCase):
    ...
    def test_delete_saved_weather_by_city_raises_error_with_city_not_in_db(self):
        self.temp.weather_database.delete = Mock()
        self.temp.weather_database.delete.side_effect = ValueError("city not in database")

        with self.assertRaisesRegex(ValueError, "city not in database"):
            self.temp.delete_saved_weather_by_city("Warsaw")

    def test_while_get_saved_weather_by_city_calls_db_find(self):
    ...
    @patch('src.weather.weather.requests.get')
    def test_get_weather_by_zip_code_pl_requests_get_should_be_called(self, mock_get):
        weather_coordinates = {
            "data": [
                {
                    "city": "Gizycko",
    ...
    @patch('src.weather.weather.requests.get')
    def test_get_random_cities_weather_returns_weather(self, mock_get):
        weather_random = {
            "data": [
                {
                    "city": "Racibórz",
        ...
        response = self.temp.get_random_cities_weather()

        self.assertEqual(response.json(), weather_random)

    @patch('src.weather.weather.requests.get')
    def test_get_random_cities_weather_server_error_returns_none(self, mock_get):
        weather_location = {
            "message": "server error"
        }
